...
const Articles = () => {
  ...
  function Items({ currentItems }) {
    ...
    <div className='arti_parent grid grid-cols-[1fr] md:grid-cols-[1fr_3fr]'>
      <button className='arti_btn flex justify-between items-center w-[90%] h-[50%] m-[20px_auto] rouned-[5px] shadow-[0px_0px_2px_0px_#9793938c] text-left p-[0px_30px] border-none bg-white md:hidden ' onClick={hamberger}><p>ক্যাটাগরি</p><p>&#x25BC;</p></button>
      <div className={click ? "acti_nav_active md:block" : "arti_nav hidden md:block"}>

        <ArticleNav handarti={handelarticale} />

      </div>
      <div className="arti_card p-[10px]">kdfksdf
      {currentItems &&
          currentItems.map((c) => (
            <div>
...
const hamberger = () =>{
  ...
  <input
    ...
    <PaginatedItems itemsPerPage={4} items={items} Items={Items} />
    </>
